...
const TASKS_POR_TICK = 2 // municípios processados por execução (teto de tempo)
...
Deno.serve(async (req) => {
  ...
  for (const task of tasks) {
    ...
    try {
      // 1) Geocode (só na 1ª vez): resolve bbox + total de células.
      ...
      if (!bbox) {
        ...
        if (!geo.ok || geo.data?.error || (typeof geo.data?.centerLat !== 'number')) {
          ...
          resultados.push({ task: task.local, skipped: true })
          continue
        }
      ...
      // 2) Um lote de células via buscar-grade (avança o cursor da task).
      ...
      if (!res.ok || res.data?.error) {
        ...
        resultados.push({ task: task.local, failed: true })
        continue
      }
