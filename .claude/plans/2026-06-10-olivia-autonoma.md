...
|---|---|---|---|
| `doces` | Confeitaria, Cafeteria | `squad_intro_doces_f/_m` | Scherbi's, Brigadayros, We Lov Cakes |
...
> parecidos com o de vocês, como Scherbi's, Brigadayros e We Lov Cakes.
...
- **Recomendado legado:** mover o número para app Meta próprio; Cloud API end-to-end
  (o `enviar-whatsapp` já envia direto). HubSpot continua como CRM via
  `hubspot-sync` (contato/deal/estágio). Só perde o inbox nativo.
- Alternativa descartada no plano antigo: responder via API de conversas do HubSpot
  (a automação já era bloqueada por escopo neste portal, automation 403).
...
- Edge Function `olivia-responder` (chamada pelo webhook a cada inbound):
  ...
  5. Cadência humana: delay 30 a 120s antes de responder, nunca instantâneo.
...
- Google Calendar API (conta stefanogebara@gmail.com, OAuth refresh token como
  secret): `freebusy` para propor 2 a 3 horários comerciais, `events.insert` com
...
- `squad_intro_doces_f` / `_m` (Confeitaria/Cafeteria) - copy com docerias + Scherbi's/Brigadayros/We Lov Cakes ("acho incrível")
